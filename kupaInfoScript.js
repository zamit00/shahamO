...
tableTesuot.innerHTML += `
  ...
  if (analysisScore && analysisScore.fields && Array.isArray(analysisScore.fields)) {
    for (const field of fieldsToCompare) {
      ...
      const avg = fieldData.average; 
      const result = fieldData.result || "";
      const valNum = (val !== undefined && val !== null && val !== '') ? parseFloat(val) : NaN;
      const avgNum = (avg !== undefined && avg !== null && avg !== '') ? parseFloat(avg) : NaN;
      const valDisplay = isNaN(valNum) ? '—' : valNum.toLocaleString('he-IL', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
      const avgDisplay = isNaN(avgNum) ? '—' : avgNum.toLocaleString('he-IL', { minimumFractionDigits: 2, maximumFractionDigits: 2 });

      tableTesuot.innerHTML += `
      <tr>
          <td style="padding:8px; border:1px solid #ccc;text-align:right">${fieldNames[field] || field}</td>
          <td style="padding:8px; border:1px solid #ccc;text-align:center">${valDisplay}</td>
          <td style="padding:8px; border:1px solid #ccc;text-align:center">${avgDisplay}</td>
  ...
  kupaInfo.innerHTML+=`<canvas id="myChartkupa" style="width:100%;max-width:1000px;max-height: 200px;"></canvas>
   ...
   speakClick();speakLater(tospeak);*/
    ...
    var hasTesuaData = true;
    for (let r = 1; r <= 12; r ++) {
        const kvutza=`tesua${r}`;
        const raw = data[0][kvutza];
        if (raw === undefined || raw === null || typeof raw !== 'string' || !raw.includes('=')) {
          hasTesuaData = false;
          break;
        }
        let parts = raw.split("=");
    ...
    if (hasTesuaData && yValues.length === 12) {
...
if (existingChart) {
    ...
    } // סוף if (hasTesuaData && yValues.length === 12)
